...
# Matching + Unmatching only — Partial Matching removed
...
st.set_page_config(page_title="Branch Reconciliation", page_icon="🔗", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');
html, body, [class*="css"] { font-family: 'Inter', sans-serif; }
.stApp { background: linear-gradient(135deg, #eef2ff 0%, #f8fafc 45%, #ecfeff 100%); }
[data-testid="stSidebar"] { background: linear-gradient(180deg, #111827 0%, #1e293b 100%); }
[data-testid="stSidebar"] * { color: white !important; }
.main-header { padding: 28px; border-radius: 24px; background: linear-gradient(135deg, #1e3a8a, #2563eb, #06b6d4); color: white; margin-bottom: 24px; box-shadow: 0 20px 45px rgba(37,99,235,0.25); }
.main-header h1 { margin: 0; font-size: 34px; font-weight: 800; }
.main-header p { margin-top: 8px; font-size: 16px; opacity: 0.95; }
.badge-row { margin-top: 18px; }
.badge { display: inline-block; padding: 8px 14px; background: rgba(255,255,255,0.18); border: 1px solid rgba(255,255,255,0.28); border-radius: 999px; font-size: 13px; margin-right: 8px; }
.kpi-card { padding: 20px; border-radius: 22px; background: rgba(255,255,255,0.78); border: 1px solid rgba(226,232,240,0.9); box-shadow: 0 14px 32px rgba(15,23,42,0.08); backdrop-filter: blur(12px); min-height: 132px; }
.kpi-title { color: #64748b; font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
.kpi-value { color: #0f172a; font-size: 32px; font-weight: 800; margin-top: 8px; }
.kpi-note { color: #64748b; font-size: 13px; margin-top: 6px; }
.section-card { padding: 22px; border-radius: 24px; background: rgba(255,255,255,0.82); border: 1px solid rgba(226,232,240,0.95); box-shadow: 0 14px 34px rgba(15,23,42,0.07); margin-bottom: 18px; }
.status-success { border-left: 7px solid #10b981; }
.status-danger { border-left: 7px solid #ef4444; }
.status-info { border-left: 7px solid #3b82f6; }
div.stButton > button:first-child { border-radius: 16px; border: none; background: linear-gradient(135deg, #2563eb, #06b6d4); color: white; font-weight: 800; padding: 0.75rem 1.2rem; box-shadow: 0 10px 24px rgba(37,99,235,0.25); }
div.stDownloadButton > button:first-child { border-radius: 16px; border: none; background: linear-gradient(135deg, #16a34a, #10b981); color: white; font-weight: 800; padding: 0.75rem 1.2rem; }
.footer { margin-top: 36px; padding: 18px; border-radius: 18px; background: #0f172a; color: white; text-align: center; font-size: 13px; }
...
<div class="main-header">
    ...
    <div class="badge-row">
        ...
        <span class="badge">❌ Unmatching</span>
...
STOP_TOKENS_LATIN = {"DATE","DOC","BANK","DEBIT","CREDIT","REF","DEP","DEPOSIT","TRANSFER","TFR","PYT","RIB","HSBC","SNB","SABB","BAB","JED","JEDDAH","RIYADH","COMPANY","CO","LTD","ACTUAL","TOSL","PAYMENT","EXIT","ENTRY","RENEWAL","FEE","CHARGE","BONUS","REWARD","RWD","DTD","BREF","B.REF","INV","FT","TT","CHQ","CHEQUE","SADAD","EXP","ERE","PAY"}
    ...
    for i, l in left.iterrows():
        ...
        for j in cands:
            ...
            if amt_diff <= tol:
                ...
                break
    match_df = pd.DataFrame([{f"{labelL} Date": a["Date"], f"{labelL} Voucher": a["Voucher"], f"{labelL} Description": a["Description"], f"{labelL} Amount": a["Amt"], f"{labelR} Date": b["Date"], f"{labelR} Voucher": b["Voucher"], f"{labelR} Description": b["Description"], f"{labelR} Amount": b["Amt"], "Amount_Diff": diff} for (a, b, diff) in matched])
...
def run_recon_core(xls_bytes, our_sheet_name, branch_sheet_name, amount_tol, name_sim_thresh, use_fast=False):
    ...
    our_name = _find_by_alias(our_sheet_name, names) or next((s for s in names if _eq(s, our_sheet_name)), None)
    if not our_name:
        raise ValueError(f"Our book sheet '{our_sheet_name}' not found. Found: {names}")
    if branch_sheet_name:
        branch_name = _find_by_alias(branch_sheet_name, names) or next((s for s in names if _eq(s, branch_sheet_name)), None)
        if not branch_name:
    ...
    m1, used_our_dr, used_br_cr = matcher(OUR_DR, BR_CR, "Our book DR", "Branch book CR", amount_tol, name_sim_thresh)
    m2, used_our_cr, used_br_dr = matcher(OUR_CR, BR_DR, "Our book CR", "Branch book DR", amount_tol, name_sim_thresh)
    matching_df = pd.concat([m1, m2], ignore_index=True) if (not m1.empty or not m2.empty) else pd.DataFrame()

    un_our = pd.concat([OUR_DR.loc[[i for i in OUR_DR.index if i not in used_our_dr]], OUR_CR.loc[[i for i in OUR_CR.index if i not in used_our_cr]]], ignore_index=True)
    un_br = pd.concat([BR_DR.loc[[i for i in BR_DR.index if i not in used_br_dr]], BR_CR.loc[[i for i in BR_CR.index if i not in used_br_cr]]], ignore_index=True)
    un_our["Which"] = "Our book"; un_br["Which"] = "Branch book"
    unmatching_df = pd.concat([un_our[["Which","Date","Voucher","Description","Debit","Credit","Amt"]], un_br[["Which","Date","Voucher","Description","Debit","Credit","Amt"]]], ignore_index=True)
    ...
    with pd.ExcelWriter(out, engine="xlsxwriter") as w:
        ...
        sheets = {"Matching": matching_df if not matching_df.empty else pd.DataFrame({"Info":["No matches found"]}), "Unmatching": unmatching_df if not unmatching_df.empty else pd.DataFrame({"Info":["No unmatching transactions"]})}
        for sheet_name, data in sheets.items():
            ...
            for idx, col in enumerate(data.columns):
                ...
                if "amount" in col_lower or "debit" in col_lower or "credit" in col_lower or "diff" in col_lower or col == "Amt":
...
def run_recon_cached_v2(file_bytes: bytes, our_sheet_name, branch_sheet_name, amount_tol, name_sim_thresh, use_fast, _version: str = "matching-unmatching-v1"):
...
with st.sidebar:
    ...
    st.divider()
    mode = st.radio("Match Mode", ["Exact Original", "Fast Same Result"], index=0)
    use_fast = mode.startswith("Fast")
    st.markdown("### 📄 Sheet Names")
    our_sheet = st.text_input("Our Book Sheet", value="Our book")
    branch_sheet = st.text_input("Branch Book Sheet", value="Branch book")
    st.markdown("### 🎯 Matching Rules")
    st.info("Amount tolerance fixed at ± 5 SAR")
    name_thresh = st.slider("Name Similarity Threshold", 0.0, 1.0, float(NAME_SIM_THRESHOLD_DEFAULT), 0.05)
    st.divider()
...
<div class="section-card status-info">
    ...
    <p class="small-muted">Upload Excel file or paste Drive / Google Sheets / OneDrive / SharePoint link.</p>
</div>
""", unsafe_allow_html=True)

source = st.radio("Choose input method", ["Upload File", "From URL"], horizontal=True)
uploaded = None
file_url = None
...
if run_btn:
    try:
        with st.spinner("Processing reconciliation..."):
            ...
            matching_df, unmatching_df, out_xlsx = run_recon_cached_v2(file_bytes, our_sheet, branch_sheet, AMOUNT_TOLERANCE_DEFAULT, name_thresh, use_fast, _version="matching-unmatching-v1")
        ...
        with c2:
            kpi_card("Matched", f"{matched_count:,}", "Matching transactions", "status-success")
        with c3:
        ...
        st.markdown("---")
        tab_summary, tab_match, tab_unmatch, tab_export = st.tabs(["📊 Summary", "✅ Matching", "❌ Unmatching", "⬇ Export"])
        with tab_summary:
            st.markdown("### 📈 Status Overview")
            chart_df = pd.DataFrame({"Status": ["Matched", "Unmatched"], "Count": [matched_count, unmatched_count]})
            if chart_df["Count"].sum() > 0:
                fig = px.pie(chart_df, names="Status", values="Count", hole=0.45, title="Reconciliation Breakdown")
                fig.update_layout(height=420, showlegend=True, margin=dict(l=20, r=20, t=60, b=20))
                st.plotly_chart(fig, use_container_width=True)
                bar = px.bar(chart_df, x="Status", y="Count", text="Count", title="Transaction Count by Status")
                bar.update_layout(height=380, margin=dict(l=20, r=20, t=60, b=20))
                st.plotly_chart(bar, use_container_width=True)
            else:
                st.info("No data available for chart.")
        with tab_match:
            st.markdown("### ✅ Matching Transactions")
            st.dataframe(matching_df if not matching_df.empty else pd.DataFrame({"Info": ["No matches found"]}), use_container_width=True, height=520)
        with tab_unmatch:
            st.markdown("### ❌ Unmatching Transactions")
            st.dataframe(unmatching_df if not unmatching_df.empty else pd.DataFrame({"Info": ["No unmatching transactions"]}), use_container_width=True, height=520)
        with tab_export:
            st.markdown("""
            <div class="section-card status-success">
                <h3 style="margin-top:0;">⬇ Export Reconciliation Report</h3>
                <p class="small-muted">Download complete Excel output with Matching and Unmatching sheets only.</p>
            </div>
            """, unsafe_allow_html=True)
            st.download_button("⬇ Download Complete Excel Report", data=out_xlsx.getvalue(), file_name=f"Branch_Recon_Output_{'FAST' if use_fast else 'ORIG'}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    except Exception as e:
        st.error(str(e))

st.markdown("""
<div class="footer">
    <strong>Branch Reconciliation v4.0</strong><br>
    Matching + Unmatching only<br>
